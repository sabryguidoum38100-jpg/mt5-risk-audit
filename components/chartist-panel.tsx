...
export default function ChartistPanel({ result }: ChartistPanelProps) {
  ...
  useEffect(() => {
    ...
    const chart = createChart(chartRef.current, {
      ...
      crosshair: {
        vertLine: { color: "#38bdf855" },
        horzLine: { color: "#38bdf855" },
      },
    ...
    const stopLossData = context.candles
      ...
      .map((candle) => ({
        time: candle.time as Time,
        value: candle.stopLoss as number,
      }));
    const takeProfitData = context.candles
      .filter((candle) => candle.takeProfit !== undefined)
      .map((candle) => ({
        time: candle.time as Time,
        value: candle.takeProfit as number,
      }));
    if (stopLossData.length > 0)
      chart
        .addSeries(LineSeries, { color: "#fb7185", lineWidth: 1, lineStyle: 2 })
        .setData(stopLossData);
    if (takeProfitData.length > 0)
      chart
        .addSeries(LineSeries, { color: "#34d399", lineWidth: 1, lineStyle: 2 })
        .setData(takeProfitData);
  ...
  const ask = async (preset?: string) => {
    ...
    try {
      const response = await fetch("/api/chat-chart", {
        ...
        body: JSON.stringify({
          question: clean,
          chartContext: chartContextForPrompt(context),
        }),
      });
      const data = await response.json();
      if (!response.ok)
        throw new Error(data?.error ?? "Erreur de l'assistant chartiste.");
      setMessages((items) => [
        ...items,
    ...
    <section className="overflow-hidden rounded-3xl border border-white/[0.08] bg-white/[0.035] shadow-2xl shadow-black/10">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-white/[0.07] px-5 py-4">
        ...
        <span className="hidden text-[11px] text-zinc-600 sm:block">
          Assistant disponible via la bulle
        </span>
      </div>
      ...
      <div className="flex flex-wrap gap-2 border-t border-white/[0.07] px-5 py-3 text-[11px] text-zinc-500">
        ...
        <span className="rounded-full bg-white/5 px-2.5 py-1">
          {context.swings.length} swings détectés
        </span>
        {context.patterns.map((pattern) => (
          <span
            key={pattern.name}
            className={`rounded-full px-2.5 py-1 ${pattern.status === "Confirmée" ? "bg-emerald-400/10 text-emerald-300" : "bg-amber-400/10 text-amber-300"}`}
          >
            {pattern.name} · {pattern.status}
          </span>
        ))}
        {latest && (
          <span className="ml-auto rounded-full bg-white/5 px-2.5 py-1">
            Dernier close {latest.close.toFixed(4)}
          </span>
        )}
      </div>
      {!drawerOpen && (
        <button
          ...
          aria-label="Ouvrir l'assistant chartiste"
          className="fixed bottom-5 right-5 z-30 flex h-14 w-14 items-center justify-center rounded-full bg-sky-300 text-zinc-950 shadow-2xl shadow-sky-400/30 transition hover:scale-105 hover:bg-sky-200"
        >
      ...
      <aside
        className={`fixed inset-x-0 bottom-0 top-auto z-50 flex h-[78vh] w-full transform flex-col border-t border-white/10 bg-black shadow-2xl transition-transform duration-300 sm:inset-y-0 sm:left-auto sm:right-0 sm:top-0 sm:h-full sm:max-w-md sm:border-l sm:border-t-0 ${drawerOpen ? "translate-y-0 sm:translate-x-0" : "translate-y-full sm:translate-x-full"}`}
      >
            ...
            {messages.length === 0 && (
              <div className="rounded-2xl border border-sky-300/15 bg-sky-300/[0.05] p-4">
                ...
                <div className="mt-4 space-y-2">
                  {[
                    "Analyse la structure de ce trade",
                    "Où est le bloc de liquidité le plus proche ?",
                    "La figure chartiste est-elle confirmée ?",
                  ].map((prompt) => (
                    <button
                      key={prompt}
                      onClick={() => void ask(prompt)}
                      className="flex w-full items-center justify-between rounded-xl border border-white/10 px-3 py-2.5 text-left text-xs text-zinc-300 transition hover:border-sky-300/30 hover:text-white"
                    >
                      {prompt}
          ...
          >
            <div className="flex items-center gap-2 rounded-2xl border border-white/10 bg-black/20 p-2">
              ...
              <button
                ...
                className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-sky-300 text-zinc-950 disabled:cursor-not-allowed disabled:opacity-40"
              >
